...
pub async fn perform_synthesis(bom_a: &BomData, bom_b: &BomData) -> SynthesisResult {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_perform_synthesis() {
        let bom_a = create_test_bom_a();
        let bom_b = create_test_bom_b();

        let result = perform_synthesis(&bom_a, &bom_b).await;
